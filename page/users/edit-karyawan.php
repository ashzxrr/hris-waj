...
if (isset($_POST['save_users'])) {
    ...
    if (isset($_POST['users']) && is_array($_POST['users'])) {
        foreach ($_POST['users'] as $uid => $ud) {
            ...
            $update_sql = "UPDATE users SET nip=?, nik=?, nama=?, jk=?, job_title=?, job_level=?, bagian=?, departemen=? WHERE id=?";
            $up_stmt = mysqli_prepare($mysqli, $update_sql);
            if (!$up_stmt) {
                $errors[] = "Gagal prepare update untuk ID {$id}: " . mysqli_error($mysqli);
                continue;
            }
            mysqli_stmt_bind_param($up_stmt, 'ssssssssi', $nip, $nik, $nama, $jk, $job_title, $job_level, $bagian, $departemen, $id);
...
$sql = "SELECT id, pin, nip, nik, nama, jk, job_title, job_level, bagian, departemen FROM users WHERE " . implode(' OR ', $conditions);
...
$foundPins = array_map(function ($u) { return (string)$u['pin']; }, $users);
$foundNips = array_map(function ($u) { return (string)$u['nip']; }, $users);
...
if (empty($users)) {
    $messages = [];
    if (!empty($missingPins)) $messages[] = "PIN tidak ditemukan: " . implode(', ', $missingPins);
    if (!empty($missingNips)) $messages[] = "NIP tidak ditemukan: " . implode(', ', $missingNips);
...
// Data for selects
$job_titles = [
    'Operator','TL Cuci','TL Cabut','TL Kedatangan','SPV Moulding','TL Moulding','GTL Moulding','GTL Cabut','Driver','Manager Produksi','SPV Kedatangan','Checker Cabut','Admin Produktivitas','Checker Moulding','TL Pengiriman','Admin','TL Packing','Superintenden','Ass. Superintenden','TL Cutter & Flek','SPV Packing','Security','Sanitasi','Purchasing/ Logistic','Maintenance','Finance Accounting'
];
...
h2{margin:18px 0 12px 0;font-size:1.25rem;color:#0b2545;font-weight:600;padding:0 20px}
...
/* Buttons */
button[name="save_users"], .btn-save{
...
button[name="save_users"]:hover{transform:translateY(-2px)}
a.cancel-link{
    display:inline-flex;
    align-items:center;
    gap:8px;
...
.user-card-body > div{display:flex;flex-direction:column}
...
@media (max-width:480px){
    h2{font-size:1.05rem;padding:12px}
    .container{padding:12px}
}

/* Accessibility: high-contrast focus */
input[type="text"]:focus, select:focus{outline-offset:2px}

/* Minor niceties */
.user-card-header small{color:var(--muted);font-weight:500;font-size:12px}
...
.btn-primary:hover { transform: translateY(-2px); box-shadow: 0 12px 30px rgba(59,130,246,0.16); }
            ...
            <div class="user-card">
                ...
                <div class="user-card-body">
                    ...
                    <div>
                        ...
                        <input type="text" name="users[<?= $i ?>][nama]" value="<?= htmlspecialchars($u['nama']) ?>" required>
                    </div>
                    <div>
                        <label>NIP</label>
